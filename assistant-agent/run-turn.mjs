...
const DEBUG_ENABLED =
  ['1', 'true', 'yes', 'on'].includes(String(process.env.ASSISTANT_DEBUG || '').toLowerCase());
...
async function runTurn(payload) {
  ...
  const emitRuntimeEvent = (event) => {
    if (finalized) {
      debugLog('post-final-stream-drop', { run_id: runId, kind: event?.kind ?? null });
      return;
    }
  ...
  const mentionedChatIds = Array.isArray(payload.mentioned_chat_ids)
    ? payload.mentioned_chat_ids.map((v) => Number(v)).filter((v) => isValidChatId(v))
    : [];
  ...
  const agent = new ToolLoopAgent({
    ...
    tools: {
      ...
      find_chats_by_contact: tool({
        description: 'Find conversations associated with a contact name or handle.',
        inputSchema: z.object({
  ...
  const runAgentPass = async (
    passPrompt,
    { streamTextDeltas = true, streamReasoningDeltas = true, passKind = 'primary' } = {},
  ) => {
  ...
  emitRuntimeEvent({ kind: 'run-finish', at_ms: durationMs, duration_ms: durationMs, run_id: runId });
  debugLog('run-finish', {
    run_id: runId,
...
function buildRequiredToolFallback({ userMessage, conversation, scopedChatIds, chatContextById }) {
...
function extractLikelySearchTerm(message) {
  const stopwords = new Set([
    'tell', 'me', 'where', 'all', 'across', 'my', 'chats', 'chat', 'conversations',
    'conversation', 'give', 'breakdown', 'of', 'about', 'spoke', 'discussed', 'mentioned',
    'mention', 'instances', 'instance', 'did', 'was', 'were', 'the', 'a', 'an', 'to', 'for',
  ]);
...
function isVagueFollowup(message) {
  ...
  if (!/\b(yes|yea|yeah|yep|look up|search|check|in my archive|do it|please)\b/.test(normalized)) {
    return false;
  }
...
function extractEntityNamesFromWindow(rows) {
  ...
  for (const row of rows || []) {
    ...
    for (const match of text.matchAll(/\b([A-Z][a-z]{2,})(?:\s+([A-Z][a-z]{2,}))?\b/g)) {
      ...
      if (['you', 'she', 'he', 'they', 'her', 'him', 'their', 'yeah', 'yep'].includes(lower)) {
        continue;
      }
      if (seen.has(lower)) {
        continue;
...
function rankInvestigationLeads({ probeOutputs, userMessage, scopedChatIds, max = 6 }) {
  ...
  for (const probe of probeOutputs || []) {
    ...
    for (const row of rows) {
      ...
      if (/\b(girlfriend|boyfriend|partner|wife|husband|dating|relationship)\b/i.test(text)) {
        score += 8;
      }
...
function buildRefinementProbes({ leads, userMessage, scopedChatIds }) {
  const probes = [];
  const queryTerms = buildQueryTerms(userMessage);
  const q = queryTerms[0] || extractLikelySearchTerm(userMessage) || userMessage.trim();
  const uniqueChats = Array.from(new Set((leads || []).map((lead) => lead.chat_id))).slice(0, 3);
  ...
  for (const chatId of uniqueChats) {
    if (Array.isArray(scopedChatIds) && scopedChatIds.length > 0 && !scopedChatIds.includes(chatId)) {
      continue;
    }
    probes.push({
      tool_name: 'search_messages',
...
function buildQueryTerms(message) {
  return String(message || '')
    ...
    .filter((token) => !['who', 'what', 'where', 'when', 'does', 'have', 'from', 'with', 'that', 'your', 'this'].includes(token))
    .slice(0, 6);
}
...
function inferCitations({
  ...
  for (const preferredRef of preferredRefs) {
    ...
    const match =
      Number.isFinite(preferredChatId)
        ? evidence.find(
            (citation) =>
              citation.chat_id === preferredChatId && citation.rowid === preferredRowid,
          )
...
function collectEvidenceCitations({ toolTraces, fallbackChatId, scopedChatIds, max = 256 }) {
  const seen = new Set();
  const out = [];
...
function normalizeReadonlySqlRows(node) {
  ...
  const columns = Array.isArray(node.columns) ? node.columns.map((v) => String(v)) : null;
...
function collectMessageEvidenceRows(node, state) {
  ...
  while (queue.length > 0 && out.length < max) {
    ...
    const effectiveChatId = Number.isFinite(nodeChatId) ? nodeChatId : inheritedChatId;
    if (isMessageEvidenceNode(value)) {
      const rowid = firstFiniteInt(value.rowid, value.ROWID, value.message_rowid);
      const chatId = Number.isFinite(effectiveChatId) ? effectiveChatId : fallbackChatId;
      if (Number.isFinite(rowid) && Number.isFinite(chatId) && isInScope(chatId, scopedChatIds)) {
...
function validateCitationIntegrity({ text, citations, toolTraces, scopedChatIds }) {
  ...
  for (const citation of citationRows) {
    if (!isInScope(citation.chat_id, scopedChatIds)) {
      return { ok: false, reason: `citation out of scope (${citation.chat_id}:${citation.rowid})` };
    }
  ...
  const evidenceKeys = new Set(evidenceRows.map((item) => `${item.chat_id}:${item.rowid}`));

  for (const citation of citationRows) {
    const key = `${citation.chat_id}:${citation.rowid}`;
    if (!evidenceKeys.has(key)) {
      return { ok: false, reason: `citation not backed by tool evidence (${key})` };
    }
  }
...
function isLikelyFactualUnit(unit) {
  ...
  if (/\b(i cannot verify|can't verify|unable to verify|insufficient evidence|need a narrower query|please @mention)\b/i.test(text)) {
    return false;
  }
...
function stripUnresolvedInlineCitations(text, citations) {
  ...
  const source = String(text || '');
  const withoutInvalidBracketed = source.replace(/\[\s*cite:(\d+):(\d+)\s*\]/gi, (full, c, r) => {
    const key = `${Number(c)}:${Number(r)}`;
    return validKeys.has(key) ? `[cite:${Number(c)}:${Number(r)}]` : '';
  });
  return withoutInvalidBracketed.replace(/\bcite:(\d+):(\d+)\b/gi, (full, c, r) => {
    const key = `${Number(c)}:${Number(r)}`;
...
function findPresentationIssue(text) {
  ...
  if (/\bchat[_\s-]*id\b/i.test(normalized) || /\bchat\s*#?\s*\d+\b/i.test(normalized)) {
  ...
  if (/\b(timestamp format|cannot be determined with.*timestamp)\b/i.test(normalized)) {
